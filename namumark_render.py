# from https://github.com/openNAMU/openNAMU/blob/beta/route/tool/func_render_namumark.py
...
class class_do_render_namumark:
    ...
    def get_tool_footnote_make(self):
        ...
        for for_a in self.data_footnote:
            ...
            if len(self.data_footnote[for_a]['list']) > 1:
                ...
                data += '<a id="' + self.doc_include + 'fn_' + self.data_footnote[for_a]['list'][
                    0] + '" href="#' + self.doc_include + 'rfn_' + self.data_footnote[for_a]['list'][
                            0] + '">(' + for_a + ') </a> '

            data += '<footnote_title target="' + self.doc_include + 'fn_' + self.data_footnote[for_a]['list'][
                0] + '">' + self.data_footnote[for_a]['data'] + '</footnote_title>'
    ...
    def do_render_text(self):
        ...
        self.render_data = re.sub(r"&#x27;&#x27;&#x27;((?:(?!&#x27;&#x27;&#x27;).)+)&#x27;&#x27;&#x27;",
                                  do_render_text_bold, self.render_data)
        ...
        # <i>
        self.render_data = re.sub(r"&#x27;&#x27;((?:(?!&#x27;&#x27;).)+)&#x27;&#x27;", do_render_text_italic,
                                  self.render_data)
    ...
    def do_render_heading(self):
            ...
            if not re.search(heading_regex, self.render_data):
                ...
                if heading_data_diff != heading_data_last:
                    ...
                    for for_a in reversed(range(1, 7)):
                        ...
                        heading_data_all = re.sub(heading_restore_regex, '<heading_' + for_a_str + '>',
                                                  heading_data_all)
                    ...
                    data_name = self.get_tool_data_storage(
                        ...
                        '' + \
                        ' <sub>' + \
                        '<a id="' + self.doc_include + 'edit_load_' + str(
                            heading_count) + '" href="/edit_section/' + str(heading_count) + '/' + url_pas(
                            self.doc_name) + '">✎</a> ' + \
                        '<a href="javascript:void(0);" onclick="javascript:opennamu_heading_folding(\'' + self.doc_include + 'opennamu_heading_' + str(
                            heading_count) + '\', this);">' + \
                        heading_folding[0] + \
                        '</a>'
                        '</sub>' + \
                    ...
                    heading_data_complete = '' + \
                                            ...
                                            ' ' + heading_data_text + \
                                            '</' + data_name + '>' + \
                                            '<div id="' + self.doc_include + 'opennamu_heading_' + str(
                        heading_count) + '" style="display: ' + heading_folding[1] + ';">' + \
                                            '<back_br>\n' + \
                                            ''
        ...
        for for_a in range(len(heading_id_data)):
            self.render_data = re.sub(heading_id_regex,
                                      '<a href="#toc" id="s-' + heading_id_data[for_a] + '">' + heading_id_data[
                                          for_a] + '.</a>', self.render_data, 1)
    ...
    def do_render_macro(self):
        ...
        def do_render_macro_double(match):
            ...
            if name_data in ('youtube', 'nicovideo', 'navertv', 'kakaotv', 'vimeo'):
                ...
                data_name = self.get_tool_data_storage(
                    '<ruby>' + main_text + '<rp>(</rp><rt>' + sub_text + '</rt><rp>)</rp></ruby>', '',
                    match_org.group(0))
        ...
        while 1:
            if not re.search(link_regex, self.render_data):
                ...
                if re.search(r'^(파일|file|외부|out):', link_main, flags=re.I):
                    ...
                    if file_out == 0:
                        file_onclick = 'onclick="javascript:document.location.pathname=\'/w/file:' + url_pas(
                            link_main_org) + '.' + url_pas(link_extension) + '\'"'
                    ...
                    if link_exist != '':
                        data_name = self.get_tool_data_storage(
                            '<a class="' + link_exist + '" title="' + link_sub + '" href="/upload?name=' + url_pas(
                                link_main_org) + '">' + link_sub, '</a>', link_data_full)
                ...
                # category
                    ...
                    if not link_main in self.data_category_list:
                        ...
                        self.curs.execute(db_change("select title from data where title = ?"),
                                          ['category:' + link_main])
                        db_data = self.curs.fetchall()
                        if db_data:
                            link_exist = ''
                        ...
                        if self.data_category == '':
                            self.data_category = '<div class="opennamu_category">' + self.get_tool_lang(
                                'category') + ' : '
                        else:
                            self.data_category += ' | '
                ...
                # inter link
                    ...
                    self.curs.execute(
                        db_change("select plus, plus_t from html_filter where kind = 'inter_wiki' and html = ?"),
                        [link_inter_name])
                    db_data = self.curs.fetchall()
                    if db_data:
                        ...
                        data_name = self.get_tool_data_storage(
                            '<a class="opennamu_link_inter" title="' + link_title + '" href="' + link_main + link_data_sharp + '">' + link_sub_storage,
                            '</a>', link_data_full)

                        self.render_data = re.sub(link_regex,
                                                  lambda x: ('<' + data_name + '>' + link_sub + '</' + data_name + '>'),
                                                  self.render_data, 1)
                    else:
                        self.render_data = re.sub(link_regex, '', self.render_data, 1)
                # out link
                    ...
                    data_name = self.get_tool_data_storage(
                        '<a class="opennamu_link_out" target="_blank" title="' + link_title + '" href="' + link_main + '">' + link_sub_storage,
                        '</a>', link_data_full)
                ...
                # in link
                    ...
                    data_name = self.get_tool_data_storage(
                        '<a class="' + link_exist + ' ' + link_same + '" title="' + link_title + '" href="' + link_main + link_data_sharp + '">' + link_sub_storage,
                        '</a>', link_data_full)
        ...
        while 1:
            ...
            if include_count_max < 0:
                ...
                if db_data:
                    ...
                    if ip_or_user(self.ip) == 0:
                        self.curs.execute(
                            db_change('select data from user_set where name = "main_css_include_link" and id = ?'),
                            [self.ip])
                        db_data = self.curs.fetchall()
                        include_set_data = db_data[0][0] if db_data else 'normal'
                    else:
                        include_set_data = flask.session[
                            'main_css_include_link'] if 'main_css_include_link' in flask.session else 'normal'

                    include_link = ''
                    if include_set_data == 'use':
                        include_link = '<div><a href="/w/' + url_pas(
                            include_name) + '">(' + include_name_org + ')</a></div>'
                    ...
                    self.data_include += [
                        [self.doc_include + 'opennamu_include_' + str(include_num), include_name, include_data,
                         'style="display: inline;"']]

                    data_name = self.get_tool_data_storage('' + \
                                                           include_link + \
                                                           '<div id="' + self.doc_include + 'opennamu_include_' + str(
                        include_num) + '"></div>' + \
                                                           '', '', match_org)
                else:
                    self.data_backlink += [[self.doc_name, include_name, 'no']]

                    include_link = '<div><a class="opennamu_not_exist_link" href="/w/' + url_pas(
                        include_name) + '">(' + include_name_org + ')</a></div>'
    ...
    def do_redner_footnote(self):
        ...
        while 1:
            ...
            if footnote_count_all < 0:
                ...
                if footnote_data[2]:
                    self.render_data = re.sub(footnote_regex, lambda x: self.get_tool_footnote_make(), self.render_data,
                                              1)
                else:
                    ...
                    if footnote_name in self.data_footnote:
                        ...
                        data_name = self.get_tool_data_storage(
                            '<sup><a fn_target="' + self.doc_include + 'fn_' + footnote_first + '" id="' + self.doc_include + 'rfn_' + footnote_num_str + '" href="#' + self.doc_include + 'fn_' + footnote_first + '">(' + footnote_name + ' (' + footnote_num_str + ')' + ')</a></sup>',
                            '', footnote_data_org)

                        self.render_data = re.sub(footnote_regex, '<' + data_name + '></' + data_name + '>',
                                                  self.render_data, 1)
                    else:
                        ...
                        data_name = self.get_tool_data_storage(
                            '<sup><a fn_target="' + self.doc_include + 'fn_' + footnote_num_str + '" id="' + self.doc_include + 'rfn_' + footnote_num_str + '" href="#' + self.doc_include + 'fn_' + footnote_num_str + '">(' + footnote_name + footnote_name_add + ')</a></sup>',
                            '', footnote_data_org)
    ...
    def do_render_redirect(self):
        ...
        if match and self.doc_include == '':
            ...
            if 'doc_from' in self.doc_set:
                data_name = self.get_tool_data_storage('<a href="' + link_main + link_data_sharp + '">(GO)</a>', '',
                                                       link_data_full)
            else:
                data_name = self.get_tool_data_storage(
                    '<meta http-equiv="refresh" content="0; url=' + link_main + link_data_sharp + '">', '',
                    link_data_full)
    ...
    def do_render_table(self):
        ...
        def do_render_table_parameter(cell_count, parameter, data, option={}):
            table_parameter_all = {"div": "", "class": "", "table": "", "tr": "", "td": "", "col": "", "colspan": "",
                                   "rowspan": "", "data": ""}
            ...
            for table_parameter in re.findall(table_parameter_regex, parameter):
                ...
                if len(table_parameter_split) == 2:
                    ...
                    if table_parameter_name == 'tablebgcolor':
                        table_parameter_all['table'] += 'background:' + self.get_tool_dark_mode_split(
                            table_parameter_data) + ';'
                    elif table_parameter_name == 'tablewidth':
                        table_parameter_all['table'] += 'width:' + self.get_tool_px_add_check(
                            table_parameter_data) + ';'
                    elif table_parameter_name == 'tableheight':
                        table_parameter_all['table'] += 'height:' + self.get_tool_px_add_check(
                            table_parameter_data) + ';'
                        ...
                        table_parameter_all['table'] += 'color:' + self.get_tool_dark_mode_split(
                            table_parameter_data) + ';'
                    elif table_parameter_name == 'tablebordercolor':
                        table_parameter_all['table'] += 'border:2px solid ' + self.get_tool_dark_mode_split(
                            table_parameter_data) + ';'
                    elif table_parameter_name == 'rowbgcolor':
                        table_parameter_all['tr'] += 'background:' + self.get_tool_dark_mode_split(
                            table_parameter_data) + ';'
                        ...
                        table_parameter_all['tr'] += 'color:' + self.get_tool_dark_mode_split(
                            table_parameter_data) + ';'
                    elif table_parameter_name == 'colcolor':
                        table_parameter_all['col'] += 'color:' + self.get_tool_dark_mode_split(
                            table_parameter_data) + ';'
                    elif table_parameter_name == 'colbgcolor':
                        table_parameter_all['col'] += 'background:' + self.get_tool_dark_mode_split(
                            table_parameter_data) + ';'
                    elif table_parameter_name == 'bgcolor':
                        table_parameter_all['td'] += 'background:' + self.get_tool_dark_mode_split(
                            table_parameter_data) + ';'
                    elif table_parameter_name == 'color':
                        table_parameter_all['td'] += 'color:' + self.get_tool_dark_mode_split(
                            table_parameter_data) + ';'
        ...
        while 1:
            ...
            if table_count_all < 0:
                ...
                for table_sub in re.findall(table_sub_regex, table_data):
                    ...
                    table_parameter['class'] = table_sub_parameter['class'] if table_sub_parameter['class'] != '' else \
                    ...
                    if table_sub[2] == '' and table_sub[3] == '':
                        ...
                        table_data_end += '<td colspan="' + table_sub_parameter['colspan'] + '" rowspan="' + \
                                          table_sub_parameter['rowspan'] + '" style="' + table_parameter['col'][
                                              table_col_num] + table_sub_parameter['td'] + '"><back_br>\n' + \
                                          table_sub_parameter['data'] + '\n<front_br></td>'

                    table_col_num += 1

                table_data_end += '</tr>'
                table_data_end = '<table class="' + table_parameter['class'] + '" style="' + table_parameter[
                    'table'] + '">' + table_caption + table_data_end + '</table>'
                table_data_end = '<div class="table_safe" style="' + table_parameter[
                    'div'] + '">' + table_data_end + '</div>'
            ...
            if middle_count_all < 0:
                ...
                if middle_slash:
                    if self.data_temp_storage[middle_slash] != '}':
                        middle_data_org = re.sub(r'<(\/?(?:slash)_(?:[0-9]+))>', '<temp_' + middle_slash + '>',
                                                 middle_data_org)
                ...
                if middle_name:
                    ...
                    if middle_name == '#!wiki':
                        ...
                        self.data_include += [
                            [self.doc_include + 'opennamu_wiki_' + str(wiki_count), self.doc_name, wiki_data,
                             wiki_data_style]]

                        data_name = self.get_tool_data_storage(
                            '<div id="' + self.doc_include + 'opennamu_wiki_' + str(wiki_count) + '"></div>', '',
                            middle_data_org)
                        wiki_count += 1
                        ...
                        self.data_include += [
                            [self.doc_include + 'opennamu_folding_' + str(folding_count), self.doc_name, wiki_data]]

                        middle_data_pass = wiki_data_folding
                        data_name = self.get_tool_data_storage(
                            '<details><summary>',
                            '</summary><div id="' + self.doc_include + 'opennamu_folding_' + str(
                                folding_count) + '"></div></details>',
                            middle_data_org
                        )
                        folding_count += 1
                        ...
                        data_name = self.get_tool_data_storage(
                            '<pre id="syntax"><code class="' + wiki_data_syntax + '">' + wiki_data, '</code></pre>',
                            middle_data_org)
                        syntax_count += 1
                        ...
                        data_name = self.get_tool_data_storage('<span style="font-size:' + wiki_size + '%">', '</span>',
                                                               middle_data_org)
                    elif middle_name in ('-5', '-4', '-3', '-2', '-1'):
                        if middle_slash:
                            middle_data_org = re.sub(r'<(\/?(?:slash)_(?:[0-9]+))>', '<temp_' + middle_slash + '>',
                        ...
                        middle_data_pass = wiki_data
                        data_name = self.get_tool_data_storage('<span style="background-color:' + wiki_color + '">',
                                                               '</span>', middle_data_org)
                    elif re.search(r'^#(?:((?:[0-9a-f-A-F]{3}){1,2})|(\w+))', middle_name):
                        if middle_slash:
                            middle_data_org = re.sub(r'<(\/?(?:slash)_(?:[0-9]+))>', '<temp_' + middle_slash + '>',
                                                     middle_data_org)
                        ...
                        middle_data_pass = wiki_data
                        data_name = self.get_tool_data_storage('<span style="color:' + wiki_color + '">', '</span>',
                                                               middle_data_org)
                    else:
                        if middle_slash:
                            middle_data += '\\'
                ...
                self.render_data = re.sub(middle_regex,
                                          lambda x: ('<' + data_name + '>' + middle_data_pass + '</' + data_name + '>'),
                                          self.render_data, 1)
    ...
    def do_render_list(self):
        ...
        while 1:
            ...
            if quote_count_max < 0:
                ...
                self.data_include += [
                    [self.doc_include + 'opennamu_quote_' + str(quote_count), self.doc_name, quote_data, '']]

                data_name = self.get_tool_data_storage(
                    '<div id="' + self.doc_include + 'opennamu_quote_' + str(quote_count) + '"></div>', '',
                    quote_data_org)
        ...
        def do_render_list_sub(match):
            ...
            return '<li style="margin-left: ' + str(
                list_len * 20) + 'px;' + list_style_data + '">' + list_data + '</li>'
    ...
    def do_render_last(self):
        ...
        if self.doc_include == '':
            if self.data_category != '':
                ...
                if ip_or_user(self.ip) == 0:
                    self.curs.execute(
                        db_change('select data from user_set where name = "main_css_category_set" and id = ?'),
                        [self.ip])
                    db_data = self.curs.fetchall()
                    category_set_data = db_data[0][0] if db_data else 'normal'
                else:
                    category_set_data = flask.session[
                        'main_css_category_set'] if 'main_css_category_set' in flask.session else 'normal'

                if category_set_data == 'normal':
                    if re.search(r'<footnote_category>', self.render_data):
                        self.render_data = re.sub(r'<footnote_category>', '<hr><' + data_name + '></' + data_name + '>',
                                                  self.render_data, 1)
                    else:
        ...
        def do_render_last_toc(match):
            ...
            if heading_data:
                ...
                self.render_data = re.sub(heading_regex, lambda x: ('<h' + heading_data + ' id="' + data + '">'),
                                          self.render_data, 1)
        ...
        if self.data_toc != '':
            ...
            self.data_toc = toc_data
            self.data_toc = re.sub(r'<toc_inside>((?:(?!<toc_inside>|<\/toc_inside>).)*)<\/toc_inside>',
                                   do_render_last_toc, self.data_toc)

            if ip_or_user(self.ip) == 0:
                self.curs.execute(db_change('select data from user_set where name = "main_css_toc_set" and id = ?'),
                                  [self.ip])
                db_data = self.curs.fetchall()
            ...
            if self.doc_include != '' or \
                ...
                self.render_data = re.sub(r'(?P<in><h[1-6] id="[^"]*">)', '<br>' + self.data_toc + '\g<in>',
                                          self.render_data, 1)
        else:
            self.render_data = re.sub(r'<toc_need_part>', '', self.render_data)
            self.render_data = re.sub(r'<toc_no_auto>', '', self.render_data)
        ...
        def do_render_last_footnote(match):
            ...
            find_regex = re.compile(
                '<footnote_title target="' + match + '">((?:(?!<footnote_title|<\/footnote_title>).)*)<\/footnote_title>')
